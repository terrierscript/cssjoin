...
CssJoin.prototype.execute = function(){
  ...
  this.parseCss(this.baseFile, relativeBase, function(err, file){
...
CssJoin.prototype.parseCss = function(cssFilePath, relativeBase, callback) {
  ...
  fs.stat(cssFilePath, function(err, result){
    ...
    fs.readFile(cssFilePath, 'utf-8', function(err, css){
      var _replaceMap = inutil.getReplaceMap(css, _resolvePaths, relativeBase);
      for(var pattern in _replaceMap.urls) {
        css = css.replace(pattern, _replaceMap.urls[pattern]);
      }
      _replaceMap = _replaceMap.imports;
      ...
      for(var key in _replaceMap){
        ...
        _this.parseCss(_childFilePath, relativeBase, function(err, childFilePath){
          children--;
          if(children > 0){
            return;
          }
