...
function getReplaceMap(css, resolvePaths, relativeBase){
  var _replaceMap = {imports: {}, urls: {}};

  // remove comment
  css = removeComments(css);
  _replaceMap.urls = getNonCssUrlMap(css, resolvePaths, relativeBase);
  ...
  for(var i=0; i < importMatches.length; i++ ){
    ...
    _replaceMap.imports[importSyntax] = file;
  }
  return _replaceMap;
}

function getNonCssUrlMap(css, resolvePaths, relativeBase) {
  var urlMap = {};
  var urlMatches = css.match(GLOBAL_URL_REGEXP);
  if(urlMatches === null){
    return urlMap;
  }
